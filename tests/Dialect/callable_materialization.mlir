...
module {
  ...
  ckl.task @layout_choice : (!ckl.tile<f32, #ckl.space<[x = 4]>>) ->
    ...
    {name = "direct", implementation_id = "layout.direct", implementation = @direct_impl,
    ...
    {name = "permuted", implementation_id = "layout.permuted", implementation = @permuted_impl,
     estimated_execution_cost = 0 : i64,
     inputs = [{name = "input", distribution = #permuted, placement = "private"}],
     outputs = [{name = "output", distribution = #permuted, placement = "private"}]}
  ]

  func.func @direct_boundary(%source: memref<4xf32>, %target: memref<4xf32>) {
  ...
  func.func @permuted_boundary(%source: memref<4xf32>, %target: memref<4xf32>) {
    %c0 = arith.constant 0 : index
